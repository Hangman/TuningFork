...
public class QoaTest extends ApplicationAdapter {
    ...
    private static final String SOUND_FILE_PATH1 =
        "42_accordion_melodious_phrase_stereo.qoa";
    private static final String SOUND_FILE_PATH2 =
        "04_electronic_gong_400hz_mono.qoa";
    private static final String SOUND_FILE_PATH3 =
        "ambience_forest_birds_mono_02.qoa";
    private Audio audio;
    ...
    public void create() {
        ...
        System.out.println(
            "SoundBuffer duration: " + sound.getDuration() + "s"
        );
        sound.play();

        // StreamedSoundSource
        streamedSound = new StreamedSoundSource(
            Gdx.files.internal(QoaTest.SOUND_FILE_PATH2)
        );
        System.out.println(
    ...
    public void render() {
        if (asyncSound == null && assetManager.update()) {
            asyncSound = assetManager.get(
                QoaTest.SOUND_FILE_PATH3,
                SoundBuffer.class
            );
            System.out.println(
                "Async SoundBuffer duration: " + asyncSound.getDuration() + "s"
            );
            asyncSound.play();
        }
    }
