...
public class ImaAdpcmWavMonoTest extends ApplicationAdapter {
    ...
    public void create() {
        audio = Audio.init();
        sound = WaveLoader.load(Gdx.files.internal(ImaAdpcmWavMonoTest.SOUND_PATH));
        final SoundSource bufferedSource = audio.obtainSource(sound);
        streamedSource = new StreamedSoundSource(Gdx.files.internal(ImaAdpcmWavMonoTest.SOUND_PATH));
