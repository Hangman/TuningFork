...
public class ProceduralSoundTest extends ApplicationAdapter {
    ...
    private static final float[] FADE_FACTORS = new float[] {
        1f,
        0.975f,
        0.95f,
        0.925f,
    ...
    private static final int SAMPLES_PER_BEAT =
        (ProceduralSoundTest.SAMPLE_RATE * 60) / ProceduralSoundTest.NPM;
    ...
    public void create() {
        audio = Audio.init();
        pcmSource = new PcmSoundSource(
            ProceduralSoundTest.SAMPLE_RATE,
            PcmFormat.FLOAT_MONO_32_BIT
        );
    ...
    private void queueNextNote() {
        ...
        final int sampleCount = (int) (ProceduralSoundTest.SAMPLES_PER_BEAT *
            songNote.durationFactor);
    ...
    private void createSineTonePcm(Note note, float[] target, int limit) {
        ...
        final float samplesPerCycle =
            ProceduralSoundTest.SAMPLE_RATE / note.getFrequency();
    ...
    private void fadeInAndOut(float[] target, int limit) {
        ...
        for (int i = 0; i < ProceduralSoundTest.FADE_FACTORS.length; i++) {
            target[i] *= ProceduralSoundTest.FADE_FACTORS[
                ProceduralSoundTest.FADE_FACTORS.length - 1 - i
            ];
        }
        ...
        for (
            int i = limit - 1;
            i > limit - ProceduralSoundTest.FADE_FACTORS.length;
            i--
        ) {
