...
public class DirectChannelRemixTest extends ApplicationAdapter {
    ...
    private static final Virtualization[] virtualizations = {
        Virtualization.OFF_DROP_CHANNELS,
        Virtualization.OFF_REMIX_CHANNELS,
        Virtualization.ON,
    };
    ...
    public void create() {
        ...
        streamedSource = new StreamedSoundSource(
            Gdx.files.internal("quadrophonic.ogg")
        );
        System.out.println(
            "Press space to switch through the different virtualization settings."
        );
    }
    ...
    public void render() {
        if (Gdx.input.isKeyJustPressed(Input.Keys.SPACE)) {
            ...
            if (
                virtualizationIndex >=
                DirectChannelRemixTest.virtualizations.length
            ) {
                virtualizationIndex = 0;
                streamed = !streamed;
            }
            final Virtualization virtualization =
                DirectChannelRemixTest.virtualizations[virtualizationIndex];
