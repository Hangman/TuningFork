...
public class LawDecoder implements WavDecoder, AiffDecoder {
    ...
    public void setup(InputStream stream, long streamLength) {
        ...
        // INPUT STREAM
        final AudioFormat.Encoding inputEncoding =
                this.encoding == Encoding.U_LAW ? AudioFormat.Encoding.ULAW : AudioFormat.Encoding.ALAW;
        final AudioFormat inputFormat =
                new AudioFormat(inputEncoding, this.sampleRate, 8, this.channels, this.channels, this.sampleRate,
                                this.bigEndian);
