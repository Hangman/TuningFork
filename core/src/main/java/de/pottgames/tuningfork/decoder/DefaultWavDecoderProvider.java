...
public class DefaultWavDecoderProvider implements WavDecoderProvider {
    ...
    public WavDecoder getDecoder(WavFmtChunk fmtChunk, boolean forStreaming) {
        ...
        final int audioFormat =
            format == WavAudioFormat.WAVE_FORMAT_EXTENSIBLE.getRegNumber()
                ? fmtChunk.getSubFormatDataCode()
                : format;
        ...
        switch (inputBitsPerSample) {
            ...
            case 8:
                if (
                    audioFormat == WavAudioFormat.WAVE_FORMAT_PCM.getRegNumber()
                ) {
                ...
                if (
                    audioFormat ==
                    WavAudioFormat.WAVE_FORMAT_MULAW.getRegNumber()
                ) {
                    return new LawDecoder(
                        channels,
                        sampleRate,
                        Encoding.U_LAW,
                        false
                ...
                if (
                    audioFormat ==
                    WavAudioFormat.WAVE_FORMAT_ALAW.getRegNumber()
                ) {
                    return new LawDecoder(
                        channels,
                        sampleRate,
            ...
            case 32:
                if (
                    audioFormat ==
                    WavAudioFormat.WAVE_FORMAT_IEEE_FLOAT.getRegNumber()
                ) {
                    return new PcmDecoder(
                        inputBitsPerSample,
                        channels,
    ...
    protected WavDecoder getAdpcmDecoder(
        ...
        if (
            audioFormat == WavAudioFormat.WAVE_FORMAT_DVI_ADPCM.getRegNumber()
        ) {
