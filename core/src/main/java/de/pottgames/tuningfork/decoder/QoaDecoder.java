...
public class QoaDecoder {
    ...
    private static final int DEQUANT_LUT[][] = {
        { 1, -1, 3, -3, 5, -5, 7, -7 },
    ...
    public QoaDecoder(InputStream stream, long samplesPerChannel)
        throws IOException, TuningForkException {
        this.stream = stream;
    ...
    private void decodeFrame(boolean firstFrame)
        throws IOException, TuningForkException {
        bufferCursor = 0;
        ...
        for (int channel = 0; channel < channels; channel++) {
            lmsStates[channel].history[0] = (short) (((rawFrameData[cursor++] &
                0xFF) <<
                8) |
                (rawFrameData[cursor++] & 0xFF));
            lmsStates[channel].history[1] = (short) (((rawFrameData[cursor++] &
                0xFF) <<
                8) |
                (rawFrameData[cursor++] & 0xFF));
            lmsStates[channel].history[2] = (short) (((rawFrameData[cursor++] &
                0xFF) <<
                8) |
                (rawFrameData[cursor++] & 0xFF));
            lmsStates[channel].history[3] = (short) (((rawFrameData[cursor++] &
                0xFF) <<
                8) |
                (rawFrameData[cursor++] & 0xFF));
            lmsStates[channel].weights[0] = (short) (((rawFrameData[cursor++] &
                0xFF) <<
                8) |
                (rawFrameData[cursor++] & 0xFF));
            lmsStates[channel].weights[1] = (short) (((rawFrameData[cursor++] &
                0xFF) <<
                8) |
                (rawFrameData[cursor++] & 0xFF));
            lmsStates[channel].weights[2] = (short) (((rawFrameData[cursor++] &
                0xFF) <<
                8) |
                (rawFrameData[cursor++] & 0xFF));
            lmsStates[channel].weights[3] = (short) (((rawFrameData[cursor++] &
                0xFF) <<
                8) |
                (rawFrameData[cursor++] & 0xFF));
        }
        ...
        for (int sampleIndex = 0; sampleIndex < samples; sampleIndex += 20) {
            ...
            for (int channel = 0; channel < channels; channel++) {
                ...
                final int sliceEnd =
                    Util.limit(sampleIndex + 20, samples) * channels + channel;
                ...
                for (int si = sliceStart; si < sliceEnd; si += channels) {
                    ...
                    final int residual = QoaDecoder.DEQUANT_LUT[scaleFactor][
                        quantizedResidual
                    ];
