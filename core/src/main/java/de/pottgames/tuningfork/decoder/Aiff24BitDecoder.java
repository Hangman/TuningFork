...
public class Aiff24BitDecoder implements AiffDecoder {
    ...
    public Aiff24BitDecoder(int bitsPerSample) {
    ...
    public int read(byte[] output) throws IOException {
        ...
        while (writeOffset < output.length) {
            ...
            while (this.bufferOffset + 1 < this.bufferLength && writeOffset < output.length - 1) {
                ...
                this.bufferOffset++; // we skip byte3 because OpenAL only supports 16-Bit integer sound (float formats = no surround sound)
