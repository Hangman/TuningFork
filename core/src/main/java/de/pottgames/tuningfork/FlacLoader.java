...
public abstract class FlacLoader {
    ...
    private static SoundBuffer load(FlacInputStream flacStream) {
        ...
        try {
            final byte[] buffer = new byte[(int) flacStream.totalSamples() * flacStream.getBytesPerSample() *
                                           flacStream.getChannels()];
            flacStream.read(buffer);
            result = new SoundBuffer(buffer, flacStream.getChannels(), flacStream.getSampleRate(),
                                     flacStream.getBitsPerSample(), flacStream.getPcmDataType());
        } finally {
    ...
    public static SoundBuffer loadReverse(FileHandle file) {
        ...
        try {
            ...
            final byte[] buffer =
                    new byte[(int) stream.totalSamples() * stream.getBytesPerSample() * stream.getChannels()];
            stream.read(buffer);
            final byte[] reversedPcm = PcmUtil.reverseAudio(buffer, stream.getBitsPerSample() / 8);
            result = new SoundBuffer(reversedPcm, stream.getChannels(), stream.getSampleRate(),
                                     stream.getBitsPerSample(), stream.getPcmDataType());
        } finally {
