...
public abstract class WaveLoader {
    ...
    public static SoundBuffer load(WavInputStream input) {
        ...
        try {
            ...
            result = new SoundBuffer(buffer, input.getChannels(), input.getSampleRate(), input.getBitsPerSample(),
                                     input.getPcmDataType(), input.getBlockAlign());
        } finally {
    ...
    /**
     * Disclaimer: This is a special use case load method. If you just want to load a wav, no matter what encoding, use
     * one of the other load methods.<br>
     * <br>
     * Loads an IMA ADPCM encoded wav file into a {@link SoundBuffer}. The file must be present on the file system (not
     * packed into a jar), be careful with this as packing everything is libGDXs default behavior. Loading is completely
     * done in native code and therefore a bit faster. Leads to a crash when fed with a wav file that is not IMA ADPCM
     * encoded.
     *
